...
void LoadProgramArguments(int argc, const char *const *argv, il::io_t,
  ...
  if (argc != 3) {
    // If th number of arguments is not equal to 3, throw an error
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (il::String(il::StringType::Byte, argv[i], il::size(argv[i])) ==
          "-i") {
        /// NEW ANALYSIS
        // We know the next argument *should* be the input filename
        argFileName = il::String(il::StringType::Byte, argv[i + 1],
                                 il::size(argv[i + 1]));

        // Check for existence and readability of the input file
        statusOfAccess = access(argFileName.asCString(), F_OK | R_OK);

        if (statusOfAccess != 0) {
          std::cerr << "Error: impossible to access to the input file: "
      ...
      }else if (il::String(il::StringType::Byte, argv[i], il::size(argv[i])) ==
          "-r") {
        /// RESTART ANALYSIS
        ...
        std::cerr << "Invalid argument "
                  << il::String(il::StringType::Byte, argv[i],
                                il::size(argv[i]))
                  << std::endl;
    ...
    std::ifstream inputRestart(argFileName.asCString());
    inputRestart >> js;
  }
}
}